// OFAuth C# SDK - Comprehensive Sandbox Test
// Tests all SDK functionality against the sandbox server
...
namespace OFAuth.Examples
{
    ...
    {
        private const string ApiKey = "sk_sandbox_gexhmn3zmvqtizdcpf2xu5dfpb2di6lygi2wmytyn55doyzwgzsxa4tboizdi3rsov2wu2jopj4di6dqnvqwq53cgi3wu4dhnyxg4nl2m5xxqm3unvthqz3jpf2ha4dcob3wonjtnntxm43emu2gu5lnmz4ho2zwortw43jtm54te5dmnvxhmzdfpjwdez3jfzswgyrvmvrdmnjxmrqti";
        private const string ConnectionId = "conn_sluhg60cp1k8uezrq2ys2d5x";
        private const string BaseUrl = "http://localhost:8789";
        ...
        static async Task TestUsersAPI(OFAuthClient client)
        {
            ...
            try
            {
                ...
                var data = await client.GetAsync<Dictionary<string, object>>("/v2/access/users/search", query);
                Success("SearchUsers");
            }
            catch (Exception ex)
            {
        ...
        static async Task TestPromotionsAPI(OFAuthClient client)
        {
            ...
            {
                var query = new Dictionary<string, string?> { { "limit", "10" } };
                var data = await client.GetAsync<Dictionary<string, object>>("/v2/access/promotions", query);
            ...
            {
                var query = new Dictionary<string, string?> { { "limit", "10" } };
                var data = await client.GetAsync<Dictionary<string, object>>("/v2/access/promotions/bundles", query);
                Success("ListBundles");
            }
            catch (Exception ex)
            {
